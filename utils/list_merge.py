...
class SubMerge:
    ...
    def sub_merge(self, url_list):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=100) as executor:
            future_to_node = {executor.submit(self.test_node, url_info): url_info for url_info in url_list}
            for future in concurrent.futures.as_completed(future_to_node):
                url_info = future_to_node[future]
                try:
                    result = future.result()
                    if result is not None:
                        ids, content = result
                        content_list.append(content)
                        print(f'Writing content of {url_info["remarks"]} to {ids:0>2d}.txt')
                except Exception as e:
                    print(f'Error occurred while testing {url_info["remarks"]}: {str(e)}')
